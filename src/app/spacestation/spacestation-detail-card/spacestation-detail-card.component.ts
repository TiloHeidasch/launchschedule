...
export class SpacestationDetailCardComponent implements OnInit {
  ...
  constructor(private service: LaunchLibraryService, public actionSheetController: ActionSheetController, public toastController: ToastController, private router: Router) {
  }
  ...
  async presentDockingLocationActionSheet(dockingLocation) {
    const actionSheet = await this.actionSheetController.create({
      ...
      buttons: [{
        ...
        icon: 'rocket',
        handler: () => {
          this.router.navigate(['launch/' + dockingLocation.docked.flight_vehicle.launch.id]);
        }
      }, {
        text: 'Goto Spacecraft',
        icon: 'planet',
        handler: () => {
          this.router.navigate(['spacecraft/' + dockingLocation.docked.flight_vehicle.spacecraft.spacecraft_config.id]);
        }
      }, {
        text: 'Cancel',
        icon: 'close',
